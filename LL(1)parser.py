...
def follow(nt):
    global start_symbol, rules, nonterm_userdef, \
        term_userdef, diction, firsts, follows, GLOBAL_FOLLOW
            ...
            if nt in subrule:
                ...
                while nt in subrule:
                    ...
                    if len(subrule) != 0:
                        ...
                        if nt != curNT:
                            if curNT in GLOBAL_FOLLOW:
                                res = GLOBAL_FOLLOW[curNT]
                            else:
                                GLOBAL_FOLLOW[curNT] = set()
                                res = follow(curNT)
...
def computeAllFollows():
    ...
    for NT in diction:
        ...
        if sol is not None:
            for g in sol:
                solset.add(g)
        GLOBAL_FOLLOW[NT] = GLOBAL_FOLLOW.get(NT, set()) | solset
        follows[NT] = solset
...
nonterm_userdef=['PROGRAM','STMTS',"STMT",'EXPR','TERM',"FACTOR",'CURR',"TEMP","EXPR'","TERM'","STMTS'","FACTOR'"]
term_userdef = ["=", "+", "-", "*", "/", "(", ")", "id", "integer", ";","++","--","^"]
